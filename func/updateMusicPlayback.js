	...
	if (globals.serverMusic[server_id].queue_pos >= globals.serverMusic[server_id].queue.length) {
		...
		globals.serverMusic[server_id].queue[queue_pos].voiceChannel.join()
			.then(connection => {
				...
				globals.serverMusic[server_id].dispatcher = connection.playStream(ytdl(globals.serverMusic[server_id].queue[queue_pos].url, {filter:"audioonly", quality: "highestaudio", highWaterMark: 1<<25}), {highWaterMark: 1, passes: 3, bitrate: 256000});
				...
				*/
				globals.serverMusic[server_id].dispatcher.stream.on("error", (err) =>{
